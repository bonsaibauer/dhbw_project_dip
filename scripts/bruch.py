...
MIN_PEAK_DISTANCE = 60  # Pixel-Abstand entlang der Kontur, in dem Ecken zusammengefasst werden
...
def get_radial_profile(contour):
    """
    Bestimmt den Schwerpunkt des Fensters und berechnet die Radien.
    """
...
def count_peaks(values, window=10, min_dist=200):
    """
    Zählt lokale Maxima (Ecken). 
    Wendet Non-Maximum Suppression an, um nahe beieinander liegende Peaks zu verschmelzen.
    """
    n = len(values)
    ...
    # 2. Alle Kandidaten finden (lokale Maxima)
    candidates = []
    lookahead = 5 
    padded = np.pad(smoothed, (lookahead, lookahead), mode='wrap')
    
    for i in range(n):
        # Index im padded array ist i + lookahead
        current_val = padded[i + lookahead]
        segment = padded[i : i + 2*lookahead + 1]
        
        # Ist es das Maximum im lokalen Fenster?
        if current_val == np.max(segment) and current_val > np.min(segment) + 2:
            # Um Mehrfachzählungen auf Plateaus zu verhindern: nur den ersten Index nehmen
            if np.argmax(segment) == lookahead:
                candidates.append((i, current_val)) # Speichere Index und Höhe
    
    if not candidates: return 0

    # 3. Filtern / Zusammenfassen (Non-Maximum Suppression)
    # Wir sortieren die Kandidaten absteigend nach ihrer Höhe (Radius).
    # Die "schärfste" Ecke gewinnt, Nachbarn werden unterdrückt.
    ...
    for idx, val in candidates:
        ...
        for kept_idx in final_peaks:
            # Berechne zyklischen Abstand (kürzester Weg im Kreis)
            dist = abs(idx - kept_idx)
            dist = min(dist, n - dist) # Wichtig für Start/Ende Übergang
...
def analyze_snack_geometry(image):
    ...
    # --- PHASE 1: Äußerer Bruch (wie gehabt) ---
    dists_outer = []
    ...
    # --- PHASE 2: Innere Struktur ---
    # WICHTIG: CHAIN_APPROX_NONE benutzen, damit Indizes = Pixelabstand sind!
    ...
    if num_windows < MIN_WINDOWS_FOR_BRUCH: return "Rest", f"Fragmentiert ({num_windows})"
    if num_windows < 6: return "Bruch", f"Zu wenig Fenster ({num_windows})"
    if num_windows > 6: return "Rest", f"Zu viele Fenster ({num_windows})"

    for idx, w_cnt in enumerate(valid_windows):
        radii, _ = get_radial_profile(w_cnt)
        if radii is None or len(radii) < 10: continue
        
        # Übergebe den Schwellwert MIN_PEAK_DISTANCE
...
# Report-Funktion bleibt weitgehend gleich, nur count_peaks Aufruf anpassen
def create_visual_report(image_paths, output_file="analysis_report.png"):
    ...
    if num_images == 1: axes = np.array([[axes[0]], [axes[1]], [axes[2]], [axes[3]]])
    ...
    for idx, img_path in enumerate(image_paths[:num_images]):
        ...
        # Innen (Profile)
        # Auch hier CHAIN_APPROX_NONE für korrekte Darstellung
        contours_all, hierarchy = cv2.findContours(mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
        inner_profiles = []
...
def sort_images(source_dir, target_dir):
    ...
    for c in CLASSES: os.makedirs(os.path.join(target_dir, c), exist_ok=True)
    ...
    for root, dirs, files in os.walk(source_dir):
        for file_name in files:
            ...
            if cat not in CLASSES: cat = "Rest"
            
            parent = os.path.basename(root)
            if os.path.abspath(root) == os.path.abspath(source_dir): name = file_name
            else: name = f"{parent}_{file_name}"

            dst = os.path.join(target_dir, cat, name)
            shutil.copy(src_path, dst)
            stats[cat] += 1
            collected_files[cat].append(src_path)
            if cat == "Bruch": print(f"   [Bruch] {name} -> {reason}")
