...
from .steps.classifier import QualityClassifier
...
def run_pipeline() -> Dict[str, object]:
    ...
    log_info("Training classifier.", progress=65.0)
    classifier.fit(feature_df[list(FEATURE_NAMES)], feature_df["target"])
    predictions = classifier.predict(feature_df[list(FEATURE_NAMES)])
    log_info("Classifier training completed.", progress=75.0)

    log_info("Evaluating predictions.", progress=80.0)
